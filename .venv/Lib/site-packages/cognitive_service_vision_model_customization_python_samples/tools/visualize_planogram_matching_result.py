...
def visualize_matching_result(img: np.ndarray, matching_result: Dict[str, Any], planogram: Dict[str, Any]) -> (np.ndarray, str):
    # Image dimensions
    ...
    for positionId, detected_obj in matching_result_dict.items():
        ...
        result = {
            ...
            'expectedProduct': product['Name'] if product else 'Unknown',
            'foundProduct': tag['name']
        }
    ...
    results['image'].append(img_str)

    results['accuracy'].append(accuracy_dict)

    json_file = json.dumps(results, indent=4)

    with open('static/Outputs/model_output.json', 'w') as file:
         file.write(json_file)
    
    print("\nModel Output JSON File Created Successfully.\n")
